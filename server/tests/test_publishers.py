...
class TestPublishersRoutes(unittest.TestCase):
    ...
    def test_get_publishers_structure(self) -> None:
        ...
        # Check that response contains only id and name fields
        required_fields = ['id', 'name']
        for publisher in data:
            for field in required_fields:
                self.assertIn(field, publisher)
            # Ensure no extra fields are present
            self.assertEqual(len(publisher.keys()), len(required_fields))

    def test_get_publishers_empty_result(self) -> None:
        """Test retrieval when no publishers exist"""
        # Clear all publishers from database
        with self.app.app_context():
